...
classifyTests <- function(tstat,cor.matrix,df.residual,p.value) {
#	Classify a series of vectors of t-test statistics into vector outcomes
#	Gordon Smyth
#	20 Mar 2003.  Last revised 21 Mar 2003.

	ngenes <- nrow(tstat)
	if(length(df.residual)==1) df.residual <- rep(df.residual,ngenes) 
	result <- tstat
	for (i in 1:ngenes) {
		o <- !is.na(tstat[i,])
		if(any(o)) result[i,o] <- classifyTests.vector(tstat[i,o],cor.matrix[o,o],df.residual[i],p.value)
	}
	result
}
